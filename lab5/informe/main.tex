...
Este informe presenta el Laboratorio 5 para la materia Aprendizaje Automático para Datos en Grafos. En este laboratorio, se implementaron y probaron distintas arquitecturas de redes neuronales en grafos (GNNs) utilizando la librería PyTorch Geometric. En particular, se implementaron capas de convolución en grafos y capas de perceptrón en grafos. Además, se entrenaron modelos GCN y GAT para el problema de clasificación de nodos en el dataset \textit{Open Graph Benchmark - ogbn-arxiv}. 

El código implementado puede consultarse en el repositorio de GitHub \url{https://github.com/j-oflaherty/AA-grafos/blob/main/lab5/Castro_OFlaherty_Lab5_AAG2025.ipynb}.
...
\section{GCN sobre \textit{Open Graph Benchmark - ogbn-arxiv}}
El dataset \textit{ogbn-arxiv} consiste en un grafo de citas entre artículos de arXiv donde cada nodo representa un artículo y cada arista representa una cita entre dos artículos. Cada paper tiene un vector de características de dimensión $d=128$ que representa el promedio de los \textit{embeddings} de texto del título y contenido. Se busca entrenar una GNN para predecir la categoría del artículo (uno entre 40 posibles) a partir de las características de los nodos y la estructura del grafo. Dividimos el dataset en conjuntos de entrenamiento, validación y test, tomando los artículos hasta 2017 para entrenamiento, los de 2018 para validación y los de 2019 para test.
...
\begin{itemize}
    ...
    \item Tamaño del filtro $K$: 3, 5.
    \item Número de capas: 3, 5.
    \item Dimensión de las características intermedias: 64, 128.    
\end{itemize}
...
\begin{figure}[htb!]
    ...
    \caption{Resultados obtenidos en el conjunto de validación para distintas combinaciones de hiperparámetros entrenando el modelo GCN.}
    \label{fig:resultados_arxiv}
\end{figure}

Se puede observar que los mejores resultados se obtienen entrenando durante 300 épocas con tamaño del filtro $K=3$, dimensión intermedia de 128 y tres capas. Con esta combinación, el accuracy obtenido en el conjunto de test fue de 72.72\%.
...
\end{equation}
La ventaja introducida por esta arquitectura está en la posibilidad de dar importancia a los vecinos de 
...
\begin{figure}[htb!]
    \centering
    \includegraphics[width=0.8\textwidth]{imagenes/resultados_gat.png}
    \caption{Resultados obtenidos en el conjunto de validación para distintas combinaciones de hiperparámetros entrenando el modelo GAT.}
...
de la figura~\ref{fig:resultados_arxiv_gat}. Por capacidad de memoria no pudimos probar distintos tamaños del filtro, fijando el mismo en $K=1$. Además, debimos reducir el número de capas de la red y las dimensiones ocultas, aunque logramos considerar la combinación que dio mejores resultados con el modelo GCN. Comparando con la figura~\ref{fig:resultados_arxiv}, vemos que no mejora el desempeño del modelo. Sin embargo, se puede notar que la mejor performance se obtiene con el modelo GAT más grande que se puede conseguir con estas combinaciones de hiperparámetros, por lo que de tener capacidad de cómputo para generar un modelo más grande se puede suponer que el desempeño mejoraría.
El mejor resultado en este caso se obtuvo entrenando durante 300 épocas, con cuatro capas y 128 dimensiones intermedias, alcanzando un accuracy de 69.5\% en el conjunto de test.

\subsection{Graph Attention v2}

La implementación inicial de Graph Attention~\cite{velickovic_graph_2018} está limitada en el sentido que
es una atención estática. Es decir, dado un nodo (\textit{query}), el orden solo depende de los valores de los vecinos (\textit{keys}).
...
Es decir, en vez de ser un único filtro lineal, se divide en un filtro para cada nodo (query) y uno para los vecinos (key). A su vez, se aplica la capa de atención luego de la no linealidad, lo que evita que toda la operación se pueda simplificar a una transformación lineal.

Al replicar el modelo de la sección anterior cambiando las capas GAT por capas GATv2, obtenemos los resultados
de la figura~\ref{fig:resultados_arxiv_gatv2}. La búsqueda de hiperparámetros se realizó tomando los mismos valores que para el modelo GAT. Al igual que en el caso del GAT, no se realizo una búsqueda de hiperparámetros exhaustiva, y se tuvo que reducir el tamaño de las capas ocultas para evitar problemas de memoria.

\begin{figure}[htb!]
    \centering
    \includegraphics[width=0.8\textwidth]{imagenes/resultados_gatv2.png}
    \caption{Resultados obtenidos en el conjunto de validación para distintas combinaciones de hiperparámetros entrenando el modelo GATv2.}
    \label{fig:resultados_arxiv_gatv2}
\end{figure}

Nuevamente, el mejor resultado se obtuvo entrenando durante 300 épocas, con cuatro capas y 64 dimensiones intermedias. Comparando con la figura ~\ref{fig:resultados_arxiv_gat}, vemos que el desempeño del modelo mejora. El accuracy obtenido en el conjunto de test fue de 70.94\%.
